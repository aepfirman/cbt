...
var jbMediaDialog = {
	...
	init : function() {
		document.getElementById("upload_target").src += '/' + tinyMCEPopup.getLang('jbmedia_dlg.lang_id', 'english');
	...
	inProgress : function() {
		...
		this.timeoutStore = window.setTimeout(function(){
			document.getElementById("upload_additional_info").innerHTML = tinyMCEPopup.getLang('jbmedia_dlg.longer_than_usual', 0) + '<br />' + tinyMCEPopup.getLang('jbmedia_dlg.maybe_an_error', 0) + '<br /><a href="#" onClick="jbMediaDialog.showIframe()">' + tinyMCEPopup.getLang('jbmedia_dlg.view_output', 0) + '</a>';
	...
	uploadFinish : function(result) {
		...
		else
		{
			document.getElementById("upload_in_progress").style.display = 'none';
			document.getElementById("upload_infobar").style.display = 'block';
			document.getElementById("upload_infobar").innerHTML = tinyMCEPopup.getLang('jbmedia_dlg.upload_complete', 0);
...
tinyMCEPopup.onInit.add(jbMediaDialog.init, jbMediaDialog);
